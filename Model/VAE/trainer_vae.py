...
class Trainer:
    ...
    def train_model(self, model, lr, epochs, train_loader, val_loader):
        ...
        for epoch in range(1, epochs + 1):
            ...
            train_loss, ssim_score, kld = self.train(model, epoch, epochs, optimiser, train_loader)
    ...
    def train(self, model, epoch, epochs, optimiser, train_loader):
        ...
        #     print(f'[VAE Trainer] Train Epoch: {epoch} / {epochs}')
        print(f'[VAE Trainer] Train Epoch: {epoch} / {epochs}')
        
        # Iterate over training batches 遍历训练数据的每个小批量
        for batch_idx, data in enumerate(train_loader):
            if (batch_idx + 1) % 50 == 0 or batch_idx == 0 or (batch_idx + 1) == len(train_loader):
                print(f"[VAE Trainer - Batch {batch_idx+1}] In progress...")
        ...
        if((epoch%50==1) or (epoch < 5) or (epoch==epochs-1)):
            ...
            sample = torch.randn(self.sample_shape).to(self.device)
