...
class Trainer:
    ...
    def test(self, model, epoch, epochs, test_loader):
        ...
        with torch.no_grad():
            # Loop through test data 遍历整个测试集
            for i, data in enumerate(test_loader):
                ...
                # 计算总损失（loss）、重建损失（recon_loss）、KL散度（kld）、
                # SSIM结构相似度指标（ssim_score）以及只考虑重建的纯损失（pure_loss）
                testloss, recon_loss, kld, ssim_score, pure_loss = model.loss_function(
                    recon_batch, data, alpha, epoch, self.params)
                
                # Accumulate loss and SSIM 累加损失与SSIM
                test_loss += pure_loss.item()           # 纯重建误差
                beta_test_loss += testloss.item()       # 包含KL散度的误差
    ...
    def early_stopping(self, counter, train_loss, test_loss, min_delta):
        # 如果测试损失明显高于训练损失
        # If the test loss is significantly higher than the training loss
    ...
    def plot_results(self, filename):
        ...
        plt.plot(loss, 'm', label = 'training loss')    # 训练损失曲线
        plt.plot(val_loss, 'g', label = 'test loss')    # 测试损失曲线
