...
Comparing VAE model training and optimization based on different strategies.

1. **RandomSearchVAE.py**
   - **Objective:** Optimize VAE model hyperparameters using a random search strategy and train the VAE model based on these parameters.
   - **Training Method:** Randomly selects multiple hyperparameter combinations.
   - **Loss Function:** Combines L1 loss, KL divergence, and SSIM/MS-SSIM.
   - **Output:** Saves each random search model and its hyperparameters, evaluating reconstruction quality (SSIM, MSE).
2. **VAE_joint_loss_mal_benign.py**
   - **Objective:** Train VAE model on **malignant and benign samples**, optimizing based on a joint loss function.
3. **VAE_MLP_joint_loss_mal_nonmal.py**
   - **Objective:** Train VAE model on **malignant and non-malignant samples**, optimizing based on a joint loss function.
4. **VAE_Dirichlet_joint_loss.py**
   - **Objective:** Replace the traditional Gaussian distribution with a Dirichlet distribution to optimize the latent space of the VAE model, focusing on joint loss optimization.
