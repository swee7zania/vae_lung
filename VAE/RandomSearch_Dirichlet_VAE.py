...
results_path = r"D:/aMaster/github_code/VAE_lung_lesion_BMVC/Data/VAE_Dirichlet"
save_results_path = r"D:/aMaster/github_code/VAE_lung_lesion_BMVC/Data/VAE_Dirichlet/VAE_params.pt"
...
# 检查 run.npy 文件是否存在，如果不存在则创建并初始化为 [1]
...
class ResampleDir(nn.Module):
    ...
    def sample(self, logits):
        ...
        dir_sample = torch.squeeze(Dirichlet(alpha_pred).rsample()) #1 # output to decoder A
        return dir_sample
...
# 检查 hyperparams_list.npy 文件是否存在，如果不存在则创建并初始化为空列表
if not os.path.exists(hyperparams_file_path):
    print("hyperparams_list.npy 文件不存在，正在初始化...")
...
def train_model(nepochs, train_loader, valid_loader, test_loader, mlp_hyperparams):
    ...
    model.to(device)
    save_results_path = r"D:/aMaster/github_code/VAE_lung_lesion_BMVC/Data/VAE_Dirichlet/MLP.pt"
